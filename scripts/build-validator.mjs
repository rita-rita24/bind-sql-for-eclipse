...
const [metadata, rawLoader, binary, worker, html] = await Promise.all([
...
const loader = rawLoader.replace(
  ...
  '// このデフォルトエクスポートは冗長に見えるが、\n  // TypeScriptからCommonJS形式のモジュールをインポートするために必要。'
);
const hash = createHash('sha256').update(binary).digest('hex');
const block = `<!-- BEGIN POSTGRESQL VALIDATOR (scripts/build-validator.mjsで生成) -->
